...
export default function TablePager<T, D>(props: TablePagerProps<T, D>) {
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (event.target.checked) {
      ...
      visibleDatas.forEach((data: any, i) => { return newSelected.push(data[Object.keys(data)[0]]) });
      setSelected(newSelected);

      const newSelectedC = visibleDatas
  ...
  const visibleDatas = React.useMemo(
    () =>
      stableSort(props.dataTotal, getComparator(order, orderBy)).slice(
        page * rowsPerPage,
        page * rowsPerPage + rowsPerPage,
      ),
    [order, orderBy, page, rowsPerPage, props.rowData],
              ...
              >
                ...
                <TableBody>
                  {visibleRows.map((row, index) => {
                    const isItemSelected = isSelected(visibleDatas[index]);
                    ...
                    return isLoading ? (
                      Array.from({ length: 5 }).map((item, index) => (
                        ...
                        >
                          ...
                          {onRenderCell(row, visibleDatas[index])}
                        </TableRow>
                      ))
                    ) :
                      (
                          ...
                          {props.hasCheckBox && (
                            <TableCell padding="checkbox">
                              <Checkbox
                                ...
                                onClick={(event) => handleClick(event, visibleDatas[index])}
                              />
                            </TableCell>
                          )}
                          {onRenderCell(row, visibleDatas[index])}
